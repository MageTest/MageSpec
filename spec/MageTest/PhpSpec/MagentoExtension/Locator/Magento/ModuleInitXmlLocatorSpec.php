...
class ModuleInitXmlLocatorSpec extends ObjectBehavior
{
    private $srcPath;
    private $specPath;
    ...
    {
        $this->shouldHaveType('MageTest\PhpSpec\MagentoExtension\Locator\Magento\ModuleInitXmlLocator');
    }
    ...
    {
        $this->supportsQuery('init:vendor_module/model')->shouldReturn(true);
    }

    function it_supports_model_query_with_submodel_name()
    {
        $this->supportsQuery('init:vendor_module/model_submodel')->shouldReturn(true);

    }

    function it_supports_model_query_with_multiple_submodel_name()
    {
        $this->supportsQuery('init:vendor_module/model_submodel_submodel_submodel_submodel')->shouldReturn(true);
    }

    function it_does_not_support_model_query_without_vendor_name()
    {
        $this->supportsQuery('init:module/model')->shouldReturn(false);
    }

    function it_does_not_support_model_query_without_module_name()
    {
        $this->supportsQuery('init:vendor/model')->shouldReturn(false);
    }

    function it_does_not_support_model_query_without_model_name()
    {
        $this->supportsQuery('init:vendor_module')->shouldReturn(false);
    }
